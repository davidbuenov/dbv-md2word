...
- **2026-08-07 - Migración a Agent Plugins 1.0.0 (versión corregida, autocontenida en `agent-plugin/`):** El servidor MCP y el Skill se empaquetan en `agent-plugin/` en la raíz del proyecto (no `.well-known/agent-plugin/`, que la especificación oficial de agent-plugins.org no menciona en absoluto — esa ruta solo tiene sentido si el plugin se sirve desde un sitio web público, y `dbv-md2word` se distribuye para instalación local). `mcp_server.py` se copió dentro de `agent-plugin/skills/dbv-md2word/scripts/` junto a `convert_md_to_docx.py`, de modo que el plugin es 100% autocontenido: `mcp.json` usa `command: "python"` (token simple, tal como exige el spec — los placeholders `${PLUGIN_ROOT}`/`${PLUGIN_DATA}` solo se expanden en `args`, `env` y `cwd`, nunca en `command`) y `args: ["${PLUGIN_ROOT}/skills/dbv-md2word/scripts/mcp_server.py"]`.
...
- **`.well-known/` no es la ubicación por defecto de un Agent Plugin:** La especificación real (agent-plugins.org) no define ningún mecanismo de descubrimiento vía `.well-known/`, cabeceras `Link` ni `robots.txt` — eso es una convención opcional documentada en la guía interna `docs/AGENT_PLUGINS.md` únicamente para proyectos que sirven el plugin desde una web pública. Para herramientas que se instalan localmente (clonando o copiando la carpeta, como `dbv-md2word`), la ubicación correcta es `agent-plugin/` en la raíz, sin el prefijo `.well-known/`.
- **Los placeholders `${PLUGIN_ROOT}`/`${PLUGIN_DATA}` no se expanden en `command`:** Solo se expanden en `args`, `env` y `cwd`. El campo `command` de un servidor `stdio` debe ser un token ejecutable simple (`python`, `node`) o una ruta relativa al propio plugin; poner un placeholder ahí produce un `mcp.json` no funcional.
...
- `agent-plugin/` → Empaquetado Agent Plugin 1.0.0 del proyecto, autocontenido (`plugin.json`, `mcp.json`, `skills/dbv-md2word/` con su propia copia de `convert_md_to_docx.py` y `mcp_server.py`). No usa `.well-known/` (instalación local, no autodescubrimiento web).
